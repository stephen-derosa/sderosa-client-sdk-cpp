...
/*
 * Receiver (publisher) for the bridge mute/unmute example.
 *
 ...
 *
 * Usage:
 *   BridgeMuteReceiver <ws-url> <token>
 *   LIVEKIT_URL=... LIVEKIT_TOKEN=... BridgeMuteReceiver
 *
...
int main(int argc, char *argv[]) {
  ...
  if (url.empty() || token.empty()) {
    std::cerr
        << "Usage: BridgeMuteReceiver <ws-url> <token>\n"
...

  {
    ...
    if (!cam_using_sdl) {
      std::cout
          << "[receiver] No camera found; sending solid-color frames.\n";
      cam_thread = std::thread([&]() {
