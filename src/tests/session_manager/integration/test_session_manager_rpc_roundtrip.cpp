...
// ---------------------------------------------------------------------------
// Test 6: Remote release of a data track.
//
// Data tracks have no mute/unmute, only release.
// ---------------------------------------------------------------------------
TEST_F(SessionManagerRemoteTrackControlTest, RemoteReleaseDataTrack) {
  skipIfNotConfigured();

  std::cout << "\n=== SessionManager Remote Release Data Track Test ==="
            << std::endl;
  ...
  auto data_track = publisher.createDataTrack("sensor-data");
  ASSERT_NE(data_track, nullptr);
  ASSERT_TRUE(data_track->isPublished());

  std::this_thread::sleep_for(2s);

  std::cout << "Requesting release on data track..." << std::endl;
  EXPECT_NO_THROW(
      controller.requestTrackRelease(publisher_identity, "sensor-data"));

  std::this_thread::sleep_for(500ms);

  EXPECT_TRUE(data_track->isReleased());

  std::vector<std::uint8_t> payload{0x01, 0x02};
  bool pushed = data_track->pushFrame(payload);
  EXPECT_FALSE(pushed) << "pushFrame should fail after remote release";
}
