...
proto::VideoCodec toProto(VideoCodec codec) {
  return static_cast<proto::VideoCodec>(codec);
}
...
VideoSource::VideoSource(VideoSource &&other) noexcept {
  *this = std::move(other);
}
...
bool VideoSource::captureEncodedFrame(const std::uint8_t *data,
                                      std::size_t size,
  ...
  if (!encoded_) {
    throw std::runtime_error(
        "captureEncodedFrame requires an encoded VideoSource");
  }
  ...
  if (!resp.has_capture_encoded_video_frame()) {
    throw std::runtime_error(
        "FfiResponse missing capture_encoded_video_frame");
  }
  return resp.capture_encoded_video_frame().accepted();
}

void VideoSource::setEncodedObserver(
    std::shared_ptr<EncodedVideoSourceObserver> observer) {
  if (!encoded_) {
    throw std::runtime_error(
        "setEncodedObserver requires an encoded VideoSource");
  }
  {
    std::lock_guard<std::mutex> lock(encoded_observer_lock_);
...
void VideoSource::registerEncodedListener() {
  ...
  encoded_listener_id_ = FfiClient::instance().AddListener(
      [this](const proto::FfiEvent &event) { handleEncodedEvent(event); });
}
...
void VideoSource::handleEncodedEvent(const proto::FfiEvent &event) const {
  ...
  case proto::EncodedVideoSourceEvent::kTargetBitrateChanged:
    observer->onTargetBitrate(
        source_event.target_bitrate_changed().bitrate_bps(),
